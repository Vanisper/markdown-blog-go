...
func articleHandler(ctx iris.Context) {
	...
	html := mdToHtml2(bytes)
	// ctx.Application().Logger().Println(html)
	ctx.ViewData("Article", html)
...
func mdToHtml2(content []byte) template.HTML {
	strs := string(content)

	var htmlFlags html.Flags = html.CommonFlags | html.HrefTargetBlank
	...
	doc := p.Parse([]byte(strs))

	// create HTML renderer with extensions
	opts := html.RendererOptions{Flags: htmlFlags}
	renderer := html.NewRenderer(opts)
	html := bluemonday.UGCPolicy().SanitizeBytes(markdown.Render(doc, renderer))
